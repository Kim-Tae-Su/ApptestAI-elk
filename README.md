...
또한 환경별 설정이 혼재되어 있어 운영 복잡도가 높았고,      
배포 및 초기 설정 과정 역시 수동으로 진행되어       
운영 효율성과 안정성 측면에서 개선이 필요한 상황이었다.    

본 프로젝트에서는 Elasticsearch 8.x 기반 보안 정책을 반영하고,    
Docker 기반 표준화된 컨테이너 환경으로 ELK Stack을 재구성하여    
운영 자동화, 보안 강화, 환경 표준화 및 서비스 운영 안정성 향상을 목표로 하였다.    
...
보안 정책을 표준화하고 장기적인 운영 안정성을 확보하였다.

또한 Elasticsearch, Logstash, Kibana를 단일 Docker 기반 환경으로 통합 구성하여
서비스 실행 환경을 표준화하고,
배포·초기 설정·환경 구성 과정을 자동화할 수 있도록 개선하였다.

단일 컨테이너 기반 구조를 통해
운영 환경 구성 단순화, 초기 세팅 비용 절감, 환경 재현성 향상 효과를 확보하였으며,
개발·운영 환경 간 설정 차이를 최소화할 수 있도록 설계하였다.

추가적으로 운영 환경에서는
보안 강화, 운영 편의성, 배포 자동화 간 Trade-off가 존재하였기 때문에,
서비스 안정성과 운영 효율을 동시에 확보할 수 있도록
컨테이너 기반 아키텍처 및 자동화 중심 구조로 설계하였다.   
