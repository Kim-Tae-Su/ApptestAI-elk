...
기존 운영 환경은 Elasticsearch 7.x 기반 노후 버전을 사용하고 있었으며,      
TLS 및 인증 정책이 부분적으로만 적용되어 보안 관리 일관성이 부족하였다.      

또한 환경별 설정 및 배포 방식이 혼재되어 있어 운영 복잡도가 높았고,      
초기 설정·인덱스 생성·Dashboard 구성 등이 수동으로 수행되어      
운영 효율성과 유지보수 안정성 측면에서 개선이 필요한 상황이었다.      

본 프로젝트에서는 Elasticsearch 8.x 기반 보안 정책을 반영하고,      
Elasticsearch·Logstash·Kibana를 단일 Docker 기반 환경으로 통합 구성하여      
배포·초기화·보안 설정 자동화를 수행하였다.      

이를 통해 운영 자동화, 보안 강화, 환경 표준화 및      
서비스 운영 안정성 향상을 목표로 하였다.      

### 기술 선정 배경
기존 ELK 운영 환경은 서버별 설정 차이와 수동 구성 요소가 많아      
환경 재현성과 운영 일관성이 낮았으며,      
버전 업그레이드 및 장애 대응 과정에서도 관리 비용이 증가하는 문제가 존재하였다.      

특히 Elasticsearch 7.x 환경에서는      
TLS 및 인증 기능이 선택적으로 적용되는 구조였기 때문에,      
서비스 운영 환경 전체에 대한 보안 정책 표준화에 한계가 있었다.      

이에 따라 기본 보안 기능(TLS, Authentication)이 강화된      
Elasticsearch 8.x 기반 구조를 도입하여      
보안 정책 및 운영 안정성을 강화하였다.      
      
또한 Elasticsearch·Logstash·Kibana를 단일 Docker 이미지로 통합 구성하고,      
컨테이너 시작 시 인증서 생성, 초기 인덱스 구성, Dashboard 생성 등      
초기화 작업이 자동 수행되도록 설계하였다.      

단일 컨테이너 기반 구조를 통해      
운영 환경 구성 단순화, 환경 재현성 향상,      
초기 세팅 및 유지보수 비용 절감 효과를 확보할 수 있었다.      

추가적으로 Elasticsearch 7.x → 8.x 메이저 버전 업그레이드 과정에서      
기존 data volume 재사용 시 내부 metadata 충돌 문제가 발생하였으며,      
이를 해결하기 위해 Snapshot + Reindex 기반 마이그레이션 전략을 적용하여      
데이터 정합성과 운영 안정성을 확보하였다.             
